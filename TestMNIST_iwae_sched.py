...
desc = 'test_conv_new_matnet_iwae_sched'
...
for eb in range(8):
    # compute the number of epochs and learning rate for this epoch block
    eb_epochs = 3**eb
    eb_lr = 0.001 * 10.**(-eb/7.)
    # acknowledge commencement of new epoch block
    eb_str = "EPOCH BLOCK {0:d}, epochs={1:d}, lr={2:.6f} **********".format(eb, eb_epochs, eb_lr)
    print(eb_str)
    out_file.write(eb_str+"\n")
    out_file.flush()

    # set learning rate for this epoch block
    lr = lrt.get_value(borrow=False)
    lr = 0.*lr + eb_lr
    lrt.set_value(floatX(lr))

    for eb_epoch in range(eb_epochs):
        epoch += 1 # acknowledge commencement of new epoch
        Xtr = shuffle(Xtr)
        Xva = shuffle(Xva)
        ...
        if (epoch % 20) == 0:
            epoch_param_file = "{0:s}/inf_gen_params_e{1:d}.pkl".format(result_dir, epoch)
            inf_gen_model.dump_params(epoch_param_file)
        ##################################
