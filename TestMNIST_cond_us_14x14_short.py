...
desc = 'test_cond_impute_2x_upsampling_14x14'
...
for i in range(depth_7x7):
    mod_name = 'td_mod_3{}'.format(alphabet[i])
    new_module = \
        GenConvPertModule(
            in_chans=(ngf * 2),
            out_chans=(ngf * 2),
...
td_modules = [td_module_1, td_module_2] + \
             td_modules_7x7 + \
...
bu_modules_7x7 = []
for i in range(depth_7x7):
    mod_name = 'bu_mod_3{}'.format(alphabet[i])
    new_module = \
        BasicConvPertModule(
            in_chans=(ngf * 2),
            out_chans=(ngf * 2),
...
# modules must be listed in "evaluation order"
bu_modules_gen = [bu_module_5, bu_module_4] + \
                 bu_modules_7x7 + \
...
# modules must be listed in "evaluation order"
bu_modules_inf = [bu_module_5, bu_module_4] + \
                 bu_modules_7x7 + \
                 [bu_module_2, bu_module_1]
...
im_modules_7x7 = []
for i in range(depth_7x7):
    mod_name = 'im_mod_3{}'.format(alphabet[i])
    new_module = \
...
im_modules = [im_module_1, im_module_2] + \
             im_modules_7x7
...
merge_info = {
    ...
    'td_mod_2': {'td_type': 'pass', 'im_module': 'im_mod_2',
...
for i in range(depth_7x7):
    td_type = 'cond'
    td_mod_name = 'td_mod_3{}'.format(alphabet[i])
    im_mod_name = 'im_mod_3{}'.format(alphabet[i])
    im_src_name = 'im_mod_2'
    bu_src_name = 'bu_mod_4'
    if i > 0:
        im_src_name = 'im_mod_3{}'.format(alphabet[i - 1])
    if i < (depth_7x7 - 1):
        bu_src_name = 'bu_mod_3{}'.format(alphabet[i + 1])
