...
g_params = inf_gen_model.all_params
...
log_p_x = T.sum(log_prob_bernoulli(
                T.flatten(Xg, 2), T.flatten(X_step[-1], 2),
                do_sum=False), axis=1)
...
test_out = test_func(train_transform(Xtr[0:100, :]))  # cheeky model implementation test


# # RESULTS RETURNED FROM APPLY_IM
# im_res_dict = {}
